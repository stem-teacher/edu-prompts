#!/bin/bash
# Build script for main_small.tex with fixed margin placements
...
# Create a temporary backup of original chapter4.tex
echo "Backing up original chapter4.tex..."
cp "$SOURCE_DIR/chapters/chapter4.tex" "$SOURCE_DIR/chapters/chapter4_original.tex"

# Copy fixed version for use
echo "Using fixed chapter4.tex with adjusted margin positions..."
cp "$SOURCE_DIR/chapters/chapter4_fixed.tex" "$SOURCE_DIR/chapters/chapter4.tex"

# Fix the \keyword{} command
echo "Fixing \keyword{} command..."
sed -i '' 's/\\keyword{/\\textbf{/g' "$SOURCE_DIR/chapters/chapter4.tex"
